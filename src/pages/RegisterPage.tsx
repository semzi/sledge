...
export default function RegisterPage() {
  ...
  const mentorshipPricing = content?.programs?.mentorship;

  const pricing = useMemo(() => {
    if (!mentorshipPricing || !selectedCurrency) return null;
    
    if (selectedCurrency === "USD") {
      const total = mentorshipPricing.priceUsd;
      return { subtotal: total, tax: 0, total, symbol: "$", code: "USD" };
    } else if (selectedCurrency === "NGN") {
      const total = mentorshipPricing.priceNgn;
      return { subtotal: total, tax: 0, total, symbol: "₦", code: "NGN" };
    }
    return null;
  }, [selectedCurrency, mentorshipPricing]);
  ...
  const handlePaystackSuccess = (registrationId: string | number, ref: any) => {
    navigate(`/payment-success?rid=${registrationId}&gateway=paystack&reference=${ref.reference}`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        cancel_url: `${CONFIG.SITE_URL}/register?canceled=1`,
      };
      ...
      if (data.payment_gateway === "stripe" && data.checkout_url) {
        ...
        initializePaystack({ onSuccess: (ref: any) => handlePaystackSuccess(data.registration_id!, ref), onClose: () => setSubmitting(false) });
      } else {
