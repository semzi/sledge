...
export default function SledgeMentorship() {
  ...
  const schemaData = {
    ...
    "@type": "Course",
    "name": "The SLEDGE Mentorship",
    "description": mentorship.heroDesc,
    "provider": {
      "@type": "Organization",
          ...
          <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-10">
            <div className="flex-1 text-left fade-in">
              ...
              <h1 className="text-4xl text-grad sm:text-5xl lg:text-7xl leading-[1.1] mb-6 tracking-tight">
                The SLEDGE <span className="text-green-500 font-bold">Mentorship</span>
              </h1>
              ...
              <ul className="space-y-4 mb-10 flex flex-col items-start">
                {mentorship.features.map((benefit: string, idx: number) => (
            ...
            {/* Price Card - Sleek & Minimalist (2/5 width on desktop) */}
            <div className="w-full lg:w-2/5 relative fade-in fade-in-delay-100">
              <div className="relative z-10 bg-white/[0.03] backdrop-blur-md rounded-[2rem] border border-white/10 overflow-hidden shadow-2xl group transition-all hover:border-green-500/30">
                <div className="p-10 md:p-12 text-center relative">
                  <div className="relative z-10">
                    <span className="text-xs font-bold mb-8 text-white/30 uppercase tracking-[0.4em] block text-center">Registration Fee</span>

                    <div className="flex flex-col items-center mb-10">
                      <div className="flex items-baseline gap-2">
                        <span className="text-7xl md:text-8xl font-semibold text-grad tracking-tighter">${mentorship.priceUsd || mentorship.price}</span>
                        <span className="text-white/40 text-lg font-medium italic">{mentorship.priceUnit}</span>
                      </div>
                      <div className="text-white/30 text-sm font-medium mt-1">
                        or <span className="text-green-500/80">₦{(mentorship.priceNgn || 0).toLocaleString()}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6 max-w-sm mx-auto">
                      <Link
                        to="/payment"
                        className="relative w-full flex items-center justify-center py-4 rounded-2xl text-white font-bold text-lg bg-green-600 hover:bg-green-500 transition-all shadow-[0_15px_30_rgba(22,101,52,0.4)] hover:scale-[1.02]"
                      >
                        {mentorship.ctaText}
                      </Link>
                      <p className="text-white/20 text-[10px] mt-8 max-w-[240px] mx-auto leading-relaxed tracking-tight font-light text-center">
                        Limited slots: {mentorship.cohortSize} participants per cohort. Applications reviewed on rolling basis.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {/* Minimalist glow */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 bg-green-500/10 blur-[100px] rounded-full pointer-events-none"></div>
            </div>
          </div>
        </section>
